...
class Api_arsip extends CI_Controller
{
    ...
    {
        $data_baca = $this->db->get_where('arsip', ['id_arsip' => $id_arsip])->row();

        if ($data_baca) {
            $file_path = 'uploads/arsip/' . $data_baca->nama_file;
            ...
            echo json_encode(['status' => false, 'message' => 'Data arsip tidak ditemukan.']);
        }
    }

    public function download($id_arsip)
    {
        $data_download = $this->db->get_where('arsip', ['id_arsip' => $id_arsip])->row();

        if ($data_download) {
            $file_path = 'uploads/arsip/' . $data_download->nama_file;
            if (file_exists($file_path)) {
