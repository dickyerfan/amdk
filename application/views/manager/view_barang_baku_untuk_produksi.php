                ...
                <div class="card-header shadow">
                    <nav class="navbar navbar-light bg-light">
                        ...
                        <div class="navbar-nav ms-auto">
                            <a href="<?= base_url('manager/barang_baku_untuk_produksi/upload') ?>"><button class="float-end neumorphic-button"><i class="fas fa-plus"></i> Tambah barang baku produksi</button></a>
                        </div>
                    </nav>
                </div>
                <div class="p-2">
                                ...
                                <table class="table table-sm table-bordered" id="example2">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">Nama Barang Jadi</th>
                                            <th class="text-center">Nama Barang Baku</th>
                                    ...
                                    <tbody>
                                        <?php
                                        $no = 1;
                                        foreach ($barang_baku_produksi as $row) : ?>
                                            <tr class="text-center">
                                                <td><?= $no++ ?></td>
                                                <td class="text-start"><?= $row->nama_barang_jadi; ?></td>
                                                <td class="text-start fw-bold"><?= strtoupper($row->nama_barang_baku); ?></td>
                                                <td><?= number_format($row->jumlah_keluar_baku, 0, ',', '.'); ?></td>
                                                <td><?= $row->input_barang_produksi; ?></td>
                                                <td><?= $row->tgl_barang_produksi; ?></td>
                                                <td>
                                                    <a href="<?= $this->session->userdata('level') == 'Admin' ? base_url('manager/barang_baku_untuk_produksi/edit/') : "javascript:void(0)" ?><?= $row->id_barang_baku_produksi; ?>"><i class="fas fa-edit text-success"></i></a>
